...
- `results` — the per-file, per-runtime verdict, with the tail of the output for every failure, and `skipped: true` on a pass that was the test's own `common.skip()` (recorded for every runtime; only node's is consulted by the scoring).
