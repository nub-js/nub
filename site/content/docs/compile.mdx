...
# Hidden from the built site until v0.10 ships `nub compile`; delete with the release.
unpublished: true
...
Large extracted programs built for the host platform on Node 24 or later also carry precompiled ESM bytecode. This increases executable and cache sizes to reduce repeated compilation. Node validates the bytecode before use and falls back to source when it cannot use the cache. Compilation does not run application code.
...
```bash
nub compile app.ts --smol                  # ~0.9 MB, finds or fetches Node
...
```console
$ nub compile app.ts
nub 0.8.3 · compiling app.ts → app
...
\e[31m\e[1merror\e[22m\e[39m  --include "data/missing.json" matched no files (the path does not exist)
```
...
nub 0.8.3 · compiling main.ts → main
...
## Asset paths keep their source-tree geometry

The extracted app dir mirrors your source tree, and every module's `import.meta.dirname` — or `__dirname`, in CommonJS — points at its own place in that mirror. So a path a module computes at run time reaches the same file it reached before compiling:

```ts
// Entry src/index.ts, helper src/shapes/lib.ts, asset at assets/icons.xml.
join(import.meta.dirname, "..", "assets", "icons.xml")        // from the entry
join(import.meta.dirname, "..", "..", "assets", "icons.xml")  // from the helper
```

Both work, and each is the path you would write for plain `node`. Write them exactly as you would without compiling.

The mirror is rooted at the deepest directory holding the entry and everything you `--include`, so including a file from outside the entry's own directory roots it higher:
...
$ nub compile src/index.ts --include assets       # `assets` is a sibling of `src`, so
                                                  # the root holds both, and the entry
                                                  # sits in `src/` inside it
```

Relative distances survive that either way, which is the point of rooting it there.

## CommonJS dependencies keep `__dirname`

The output is an ES module, but a CommonJS dependency bundled into it still has `__dirname` and `__filename`. Your own code gets the mirrored directory described above. A dependency does not: bundling dissolves its `node_modules` directory, so there is nothing to mirror, and both names resolve against the entry's directory instead.
...
Reading that gives `ENOENT` — an error the code can catch, rather than the crash it used to be. `--include` does not help here, because it embeds the file at its own source-tree path rather than beside the entry. A package that reads a file it ships this way needs [`--external`](#--external), which leaves it installed and unbundled.
...
The addon is extracted under the same `node_modules` layout it was installed in and loaded from there, so the platform's dynamic loader gets a real file at a real path — the same thing it gets from an ordinary install.
...
Shipping 1 package unbundled, in its own installed layout:
  @napi-rs/nice — it declares 16 per-platform binary packages (napi-rs layout)
    ...
    \e[2moutput\e[22m  \e[96mapp\e[39m  29.5 MB\e[2m  (node 28.2 MB · app 165 KB · launcher 851 KB · signature 228 KB)\e[22m
...
\e[32m\e[1m✓\e[22m\e[39m compiled \e[96mapp\e[39m in \e[2m3.2s\e[22m
```
...
Remove debugging calls at build time. Repeatable, one category per use:

```bash
nub compile app.ts --drop console --drop debugger
```

Two categories exist: `console` removes every `console.*()` call, `debugger` every `debugger` statement.

A dropped call is not evaluated, so whatever you passed to it goes too — `console.log(buildReport())` takes `buildReport()` with it. Where a call's result is read, the call becomes `undefined`, which is what `console.log` returned anyway.

Dropping rides the minifier, which is on by default. Asking for it alongside `--no-minify` is refused rather than quietly leaving every call in place.
...
Set the fields Windows shows in an executable's Details tab, and that `(Get-Item app.exe).VersionInfo` reads:

```bash
nub compile app.ts --platform win32-x64 --metadata "CompanyName=Acme Inc." --metadata "LegalCopyright=Copyright 2026 Acme"
```

Most builds need no flag at all. Product name, description, version and company come from `name`, `description`, `version` and `author` in the nearest `package.json`, and the original filename comes from `--out`. The flag only overrides that, and `Key=` with an empty value drops a field rather than writing a blank one.

Windows defines the field names, so nub accepts that set and no more: `Comments`, `CompanyName`, `FileDescription`, `FileVersion`, `InternalName`, `LegalCopyright`, `LegalTrademarks`, `OriginalFilename`, `PrivateBuild`, `ProductName`, `ProductVersion`, `SpecialBuild`. Anything else is refused at the start of the build — a name Windows does not know would encode fine and then show up nowhere, which costs a full build to notice.

A version like `2.5.1-rc.3` reaches the `FileVersion` string as typed. Windows also keeps a separate numeric version of four 16-bit numbers, which has nowhere to put a prerelease tag, so that one reads `2.5.1.0`.

Like `--icon`, this works while cross-compiling — a Windows binary built on macOS gets its metadata like any other — and is refused for a non-Windows target, because no other executable format carries these fields.

### `--hide-console`

Start a Windows executable with no console window — what a GUI app, a tray icon, or a file-association handler needs:

```bash
nub compile app.ts --platform win32-x64 --hide-console
```

Launched from Explorer, an ordinary compiled binary opens a console window beside the application and keeps it there until the program exits. With this flag, nothing appears.

The same binary run from a terminal still writes to it. Suppression applies only where there is no console to inherit, so a build made for double-clicking stays usable from `cmd.exe` and PowerShell — though `cmd.exe` returns to the prompt without waiting, as it does for any Windows GUI program.

Nothing printed reaches a user who double-clicks, so a hidden binary cannot report a startup failure to the screen, and the first-run setup notice stays silent as well. Give the application a channel of its own — a log file, the event log, a message box — before shipping one.

Like `--icon`, this works while cross-compiling and is refused for a non-Windows target. A console window is a Windows concept, and neither the macOS nor the Linux executable format carries anything that would suppress one.

### `--metafile`

Write a report describing what went into the bundle:

```bash
nub compile app.ts --metafile report.json
```

The report is JSON in [esbuild's metafile schema](https://esbuild.github.io/api/#metafile), so it opens in the tools already written for that format — esbuild's own `analyzeMetafile`, esbuild-visualizer, bundle-buddy. It has two keys: `inputs`, every module the bundler read with its source length, module format and imports; and `outputs`, every file the bundler emitted with its size and how many bytes each input contributed to it.

The per-input counts are measured before minification while an output's own `bytes` is the file after it, so with minification on the parts sum higher than the whole. Rank inputs against each other with them; do not expect them to account for the final size. They line up under `--no-minify`, unless the output carries an inline source map — that map counts toward the file and belongs to no input, and it is usually larger than the code it describes.

The report covers the bundled JavaScript and nothing else. It says nothing about the embedded Node runtime — usually the largest part of the binary, and what `--smol` leaves out — nor about files added with `--include`, nor about native addons carried unbundled.
...
#### Scoping it to one directory

Bare, the flag excuses every computed import in the program. Give it a glob and it excuses only the ones written in files that glob names, so the rest of the program keeps failing the build:

```bash
nub compile app.ts --allow-dynamic-import 'src/plugins/**'
```

Repeat it for more than one directory. The glob is matched against the path of the file the `import()` is written in, relative to the directory you run the command from — the same anchor `--include` uses. A file outside that directory, a dependency in particular, matches no relative glob and stays refused.

Scoping by the importing file rather than by the specifier is deliberate. A computed specifier has no shape to match on — that is what makes it computed, and what the flag is for — while the file it is written in is always known, and is the part you can vouch for.
...
The app's extracted files are validated before publication, not scanned on every launch. Later runs check the private cache directory, completion marker, and entry file. If other extracted files are modified or deleted afterward, remove that app's extraction directory to restore them on the next run.
