...
```jsonc title="nub.jsonc" full
{
  ...
  "prefix": "dotenvx run --",              // a wrapper in front of file runs, scripts, and watch
...
Five fields take paths: [`preload`](#preload), [`envFile`](#envfile), [`tsconfig`](#tsconfig), plus [`nodeExecutable`](#nodeexecutable) and [`prefix`](#prefix) in their path forms. A relative path resolves against the directory of the file that set it — not your working directory.
...
Name the Node binary to run, instead of letting Nub resolve a version.
...
Put a command in front of everything Nub runs for the project.
...
The prefix applies to a file run, a `package.json` script, and `nub watch`:
...
A script is wrapped as a whole, so a script that starts no Node process still runs behind the prefix. The prefix is set once per project: a `nub run` inside a script, or a wrapper written in Node, does not wrap the same project again. Tools that run someone else's CLI — `nubx` and [`nub dlx`](/docs/nubx#nub-dlx) — do not take it.

The program resolves from the project's `node_modules/.bin`, then `PATH`, so a wrapper installed as a devDependency works with no global install. A path is also accepted, absolute or relative to the file:

```jsonc title="nub.jsonc"
{
  // ...
  "prefix": ["./tools/wrap", "--quiet"]
}
```

The Node process still receives [environment files](/docs/runtime/env) as before; on a file run the wrapper starts with them set too. A wrapper that owns the environment itself pairs the prefix with `"envFile": false`. Compatibility mode (`--node`) runs without the prefix.
...
Load no environment at all with `false`. This covers [Varlock](/docs/runtime/varlock) too, so a project with a `.env.schema` hands its program an empty environment.
...
Paths expand `${VAR}` and `$VAR` against the environment, which a config file never gets from the shell, and resolve as described under [relative paths](#relative-paths).
