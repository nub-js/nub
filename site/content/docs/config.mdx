...
Five fields take paths: [`preload`](#preload), [`envFile`](#envfile), [`tsconfig`](#tsconfig), and [`nodeExecutable`](#nodeexecutable) and [`prefix`](#prefix) in their path forms. A relative path resolves against the directory of the file that set it — not your working directory.
...
Nub still loads [environment files](/docs/runtime/env) before it starts the wrapper. A wrapper that owns the environment itself pairs the prefix with `"envFile": false`.
