...
The Nub CLI matches a `nub` condition on every run it augments, so a package that publishes a branch for it resolves there. The name follows the same [runtime keys](https://runtime-keys.proposal.wintertc.org/) convention `bun`, `deno` and `workerd` use.
...
Two runs do not match it. Passing `--node` runs with Node's own conditions and nothing else, and the standalone [loader](https://www.npmjs.com/package/@nubjs/loader) leaves Node's condition set alone so a file resolves the same way under it as under tsx. Both fall through to `default`.
