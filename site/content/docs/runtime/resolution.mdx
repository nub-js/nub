...
Nub is registered in the WinterTC [runtime keys](https://runtime-keys.proposal.wintertc.org/) report alongside `bun`, `deno` and `workerd`. Every augmented run matches the `nub` condition, so a package that publishes a branch for it resolves there.
...
Compat mode is the exception. Passing `--node` runs with Node's own conditions and nothing else, so the same import resolves to `default`.
