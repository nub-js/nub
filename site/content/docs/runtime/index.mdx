...
On Linux, direct Node launches can use a 16 MiB V8 semi-space when the process has a 256–512 MiB cgroup memory limit. This includes file runs and Node-backed `nub exec`/`nubx` binaries. The initial supported releases are Node 22.23.2, 24.20.0, and 26.8.1; other releases keep Node's defaults. This changes the main heap only. Workers retain their own defaults and explicit `resourceLimits`.

Nub skips this tuning when Node options are supplied through argv, `NODE_OPTIONS`, or project configuration, or when PnP or an external environment loader owns startup. Unknown memory limits, watch mode, compiled executables, and inherited augmented processes keep their existing behavior. Application arguments after the entry path do not disable tuning.

The larger nursery can reduce garbage collection for allocation-heavy workloads, but it is not a process memory cap. Workers, native allocations, and other processes can share the same container budget. Use [explicit V8 flags](/docs/config#v8flags) for an application-specific budget, or [compatibility mode](#compatibility-mode) for unchanged Node defaults.
