...
- **Threadpool** — libuv's threadpool, which serves `fs`, `dns.lookup`, and async `zlib` and `crypto` calls, is sized to the cores available to the process, with a floor of 4, instead of Node's fixed 4. A container's thread limit caps it, and Windows stops at 8. On Linux the threads beyond the first four run at a lower priority, so they take only the CPU that nothing else on the machine is using. A process your app spawns, including a cluster worker, starts with Node's default unless it runs through `nub`. A `UV_THREADPOOL_SIZE` you set, in the shell or in an env file, is used as is and is inherited as under Node.
