...
- **Threadpool** — libuv's threadpool, which serves `fs`, `dns.lookup`, and async `zlib` and `crypto` calls, is sized to the cores available to the process, with a floor of 4, instead of Node's fixed 4. A `UV_THREADPOOL_SIZE` you set is used as is.
