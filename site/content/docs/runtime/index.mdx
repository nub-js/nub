...
On Linux, direct Node launches can use a 16 MiB V8 semi-space when the process has a 512 MiB cgroup memory limit. This includes file runs and Node-backed `nub exec`/`nubx` binaries. The supported releases are Node 22.23.2, 24.20.0, and 26.8.1; other releases and memory budgets keep Node's defaults. This changes the main heap only. Workers retain their own defaults and explicit `resourceLimits`.
