...
`execArgv` is what decides the container. Nub's runtime flags reach a compiled artifact through that field, and an artifact that could not carry them would diverge from `nub <file>` on the first program that uses one — so a target Node below the backport bands keeps the launcher, as does any payload that needs real filesystem paths (`--external` packages, a retained computed `import()`, `--include`d files, native addons, a linked source map). A payload that re-enters its own executable keeps it too: `node:cluster` re-runs the entry, and `child_process.fork` names a module that Node discards for a single-executable, so both need the real Node a launcher extracts. Which payloads those are is read off the emitted chunks as resolved module specifiers rather than as text, because `cluster` and `child_process` are both ordinary enough words to appear in a message or an unrelated export name. `--smol` is never a single-executable application: one *is* a Node, and the sub-1 MB shape embeds none.
...
4. `acquire_node` extracts or provisions Node when no external `--smol` Node was found. `ensure_app` extracts the bundled entry and assets. The staged app tree is flushed and validated against the payload's names, recorded lengths and executable bits before atomic publication. A warm run checks the private app directory and its parent, the completion marker, and the entry file without enumerating the tree. Missing markers or entries trigger re-extraction; changes to other files within a completed cache do not. Removing the marker or the extraction directory requests repair. The embedded Node retains its executable and length checks.
...
A worker payload runs in a single-executable application too, and it needs one thing the main thread does not. Module hooks are registered per thread, so a worker started on a payload URL cannot load it: the hooks that serve that URL never ran in the new realm. The single-executable loader therefore replaces `Worker` itself. A target naming a payload file starts on a generated bootstrap that registers the same hooks and then imports the file; every other target is passed through unchanged, so a worker whose entry really is a file on disk still reaches that file. The replacement is built only for a payload that carries a worker, because reading `node:worker_threads` to build it loads that builtin on every start. Both spellings reach it: `node:worker_threads` hands over the URL the chunk built, while the global `Worker` converts that URL to a path before it delegates. The blob carries the bootstrap as an asset as well as as its main, since the wrapper the worker runs has to install it in a realm that inherits nothing from the first.
