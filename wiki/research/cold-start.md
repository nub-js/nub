...
| `InitializeOncePerProcessInternal` (OpenSSL config load 2.5%, CSPRNG seeding 3.7%, `V8::Initialize` 1.4%) | 12.6% |
| `LoadEnvironment` (running `internal/main/eval_string`) | 8.1% |
| Isolate and heap teardown at exit | ~5% |
| Kernel page faults (3,196 minor faults, two thirds of them anonymous pages V8 zeroes while deserializing; the file-backed ones batch 16 pages per fault) | 17.9% |
...
Two things that look like wins and are not, measured on the same box. Pre-loading `stream`, `net` and `tty` into a snapshot removes the 28-module load behind the first `console.log` (3.09 → 0.18 ms) but grows the snapshot by 660 KB and its deserialization by ~2 ms, so `hello.js` gains 0.5 ms while a script that never prints loses 2.1 ms; the built-in modules are already compiled from the embedded code cache, and having `net` import `internal/streams/duplex` instead of the `stream` index is not an option at all, because the Readable operators (`map`, `filter`, `toArray`, …) are installed on the prototype only by `lib/stream.js`, so sockets created before anything loads the index would lack them. And `-z pack-relative-relocs` (RELR) would trim the loader's 24% share of `node --version`, but it needs glibc 2.36 at run time while the official binaries target 2.28.

On macOS the last framework cost is attributable: a binary that links nothing but libSystem starts in 4.19 ms on this host, one that also links CoreFoundation in 5.48 ms (+0.6 ms of dyld work, +0.6 ms of initializers), and adding Security on top changes nothing further. Node imports 37 CoreFoundation and Security symbols: 32 from the system certificate store code in `src/crypto/crypto_context.cc`, which only runs under `--use-system-ca`, and five from abseil's `cctz::local_time_zone()` (the time-zone lookup plus three `CFString` helpers). Deno removed the same dependency in 2.9 by linking dlopen-on-first-use shims for its 47 framework imports ([lzld][denolzld]); Node's own 2022 look at dropping CoreFoundation ([#44715][issue44715]) was closed because Instruments' heap-allocation recorder needed the framework loaded.

### Inside Node's own tree: seven more candidates, built and measured

A second pass stayed inside `src/`, `lib/` and the build, where a change can land as an ordinary pull request.

Each candidate was built against an unpatched build of the same tree and measured interleaved on an idle Linux x64 box (Node main at `f85bb5ed`, which includes [#65336][pr65336]).

| Change | Saving | Where it shows |
| --- | --- | --- |
| Worker stdio stream classes created on first use (`lib/internal/worker/io.js`), with the two per-Worker callers that forced them fixed | Worker `environment → bootstrapComplete` 5.91 → 4.68 ms median; `new Worker()` to exit 15.7 → 13.7 ms; 30 sequential Workers 485 → 423 ms; Node's `misc/startup-core` worker mode +15% | every Worker spawn; 19 builtins leave the worker's module list; main thread unchanged |
| abseil compiled with `NDEBUG` in release: `tools/v8_gypfiles/abseil.gyp` was the one V8 target without `features.gypi`, so `absl::Mutex`'s deadlock detector allocated a 1 MiB arena on the first mutex of every process, before Node's call that disables it | `nodeStart → v8Start` 3.20 → 2.56 ms; 259 fewer page faults; binary 195 KiB smaller; also removes an ODR split on `absl::Mutex::~Mutex` between V8 and abseil translation units | every process, every platform |
| `internal/fs/sync_write_stream` taking `Writable` from `internal/streams/writable` instead of the `stream` index, and `readable` deferring `string_decoder` and `internal/streams/from` | `hello.js` with stdout redirected to a file 30.28 → 29.28 ms median (13 builtins fewer); `--no-node-snapshot` −4.1 ms | scripts that print to a file or `/dev/null`; the pipe path loses 2 builtins; `-e 0` unchanged. Same shape as [#65329][pr65329] |
| CLI options read from C++ one key at a time instead of serializing all 369 entries (twice, for a file script) | binding time 0.24 → 0.10 ms (`-e 0`) and 0.42 → 0.10 ms (`file.js`); bootstrap phase −0.4 ms | inside `environment → bootstrapComplete`; the end-to-end wall-clock delta did not resolve above noise. Same function as [#52451][pr52451] |
| Option help text and namespace ids held as string views | ~0.1 ms, at the measurement floor | every process, including `node --version` |

Two measured non-wins. Creating the 316 error classes on first access shrinks the main-context snapshot by 12.6% and its deserialization by 0.52 ms, yet `node -e 0` is 1.4–1.9 ms slower in every round: deserialization costs about 3 µs per object, 26 KB of flat integer arrays cost nothing measurable, and the getters that replace the classes run at startup instead (the same wall [#65329][pr65329] records for an intermediate version that took builtins out of the snapshot). And deferring more of the bootstrap `require` list saves nothing on a snapshot build, because `node file.js` compiles exactly one builtin at run time and `-e 0` four; every other entry in `process.moduleLoadList` is deserialized.

Two costs were re-priced. C++ static initializers run before `main` and cost about 0.7 ms of every Linux process, about 20% of `node --version`: 0.4 ms for the option-parser tables in `src/node_options.cc` (632 entries, 390 of them copies made by `Insert()`) and 0.3 ms for the generated builtin source map in `node_javascript.cc`, each measured by building a second unused copy and diffing 1,000 interleaved runs. Function-local statics would save nothing, since every process parses `argv` through the outermost parser and constructs the builtin loader. And the page-fault share is 17.9% of samples, not 23%: two thirds of the 3,196 faults are anonymous pages V8 zeroes during deserialization (`clear_page_erms` alone is 5.9% of samples), the file-backed third batches 16 pages per fault and costs about 1%, and prefaulting would move that 1% rather than remove it. `src/large_pages/` was removed by [#65389][pr65389] after `--use-largepages=on` measured +107% wall clock on Node 24 ([#64408][issue64408]).

One measurement trap is worth recording. Two builds of byte-identical source, linked separately, differed by up to 1.5 ms in the minimum of `node -e 0` on the same box, and a non-interleaved run list reversed the sign of a 0.5 ms effect. Sub-millisecond claims need several builds per side, interleaved, with a null control.
...
A proposal to remove CoreFoundation ([#44715][issue44715]) was closed in 2022 after screenshots on macOS 12.6 showed Instruments' Heap Allocations recorder no longer working without the framework loaded; nothing in that thread concerns ICU. Re-tested on macOS 15.7.9 and 26.5.2: malloc stack logging records the same 80,000 call stacks with the same symbolicated frames whether CoreFoundation is linked eagerly, delay-loaded or not linked, with the eager build as the positive control, and the Allocations instrument records all three. Apple's linker has shipped `-delay_framework` since Xcode 15, within Node's Xcode 16.4 floor, so the framework can be bound on first use with two link flags and no shim; a small binary saves 0.8–1.0 ms per start that way and pays about 1 ms on the first call that does use the framework (`--use-system-ca`, `fs.watch`). Built into Node on a macOS 15 runner, the two flags take the dyld initializer count from 659 to 261 and the pre-`main` phase down by about 0.4 ms on `--version`, `-e 0` and `hello.js` (median of twelve alternated rounds of 40 spawns, timed by an interposed dylib; the patched build is faster in 8, 11 and 10 of the 12). An un-instrumented wall clock on the same runners does not resolve it (hyperfine min of 200 within 0.1 ms), and `performance.nodeTiming` cannot see it at all, since its origin is a static initializer of the executable that dyld runs after every dylib has initialized; with the interposer in `DYLD_INSERT_LIBRARIES` the launch reads about 0.7 ms faster end to end, most likely because the variable turns off dyld's prebuilt loader and the framework tree then costs more to load. `--use-system-ca` still returns the 121 system roots and the tls and crypto suites pass. The frameworks load again on first use, all 659 initializers: a first `fs.watch` pays about 1.2 ms because libuv opens CoreFoundation on the main thread for FSEvents, a blocking `tls.getCACertificates('system')` about 0.6 ms, and `--use-system-ca` alone nothing, since its read runs on its own thread. Those three figures are the framework-binding penalty only, and the last of them holds only while the off-thread read finishes before anything needs its result — which is a property of the keychain, not of the flag. On a populated developer keychain the read itself is measured below in seconds, and the main thread then waits for it.

**The read is cheap on a clean runner and expensive on a real machine, and the difference is the number of certificates.** `ReadMacOSKeychainCertificates` in `src/crypto/crypto_context.cc` asks `SecItemCopyMatching` for every certificate in every keychain — `kSecMatchLimitAll` over `kSecClassCertificate`, not the roots — and then calls `IsCertificateTrustValid` on each, which builds a `SecPolicyCreateSSL` policy and runs a full `SecTrustEvaluateWithError`. That is one `trustd` round trip per certificate, and a trust evaluation may attempt AIA/OCSP/CRL fetches, so part of the cost is network rather than CPU. Measured on node v26.7.0 on a developer Mac, `tls.getCACertificates('system')` costs **2945 ms** against 47 ms for a run that does not read the system store, and the flag adds **2235 ms** to a real application's first TLS use (657 ms → 2892 ms, min of 7). The same mechanism is reported at 8.5–9.8 s on corporate-managed Macs in [anthropics/claude-code#53660][ccissue53660]. Four upstream changes have aimed at the flag's cost — cert loading off-thread ([#59550][pr59550]), only when TLS is used ([#59856][pr59856]), per-env rather than per-process ([#60678][pr60678]) and a skip-condition fix ([#60764][pr60764]) — and v26.7.0 still pays it, because they move *when* the read happens rather than what it does per certificate.

From Daniel Lemire in [#180][perf180]:
...
- **CoreFoundation dependency on macOS** — [#44715][issue44715] proposed removal; closed in 2022 because Instruments' Heap Allocations recorder needed the framework loaded. That observation does not reproduce on macOS 15.7 or 26.5, and `-delay_framework` gives most of the saving without removing the dependency.
...
[ccissue53660]: https://github.com/anthropics/claude-code/issues/53660
[pr59550]: https://github.com/nodejs/node/pull/59550
[pr59856]: https://github.com/nodejs/node/pull/59856
[pr60678]: https://github.com/nodejs/node/pull/60678
[pr60764]: https://github.com/nodejs/node/pull/60764
...
[pr65329]: https://github.com/nodejs/node/pull/65329
[pr52451]: https://github.com/nodejs/node/pull/52451
[pr65389]: https://github.com/nodejs/node/pull/65389
[issue64408]: https://github.com/nodejs/node/issues/64408
...
- 2026-09-05 — Seven candidates inside Node's own tree built and measured (Worker stdio streams on first use, abseil built with `NDEBUG`, the stream tree behind stdout-to-file, per-key CLI option reads, string-view option tables; lazy error classes and further bootstrap deferral measured as non-wins). Corrections: page faults are 17.9% not 23% and two thirds anonymous; `--use-largepages` is gone; static initializers cost 0.7 ms before `main`; the `net → duplex` shortcut is unsafe; Security is linked alongside CoreFoundation; the 2022 Instruments blocker does not reproduce on macOS 15/26, and delay-loading the two frameworks in a Node build is worth about 0.4 ms of pre-`main` time per process by an interposed phase split, unresolvable by a plain wall clock on the runners, at a one-time cost of about 1.2 ms to a process that calls `fs.watch`; `performance.nodeTiming` is structurally blind to the change.
- 2026-09-08 — `--use-system-ca` is not free on a populated keychain: `tls.getCACertificates('system')` measures 2945 ms against 47 ms without, and the flag adds 2235 ms to an application's first TLS use. The mechanism is a full `SecTrustEvaluateWithError` per certificate over every keychain, so the four upstream changes that moved the read off-thread do not remove it. The prior "`--use-system-ca` alone nothing" figure is retained and scoped: it is the framework-binding penalty on a clean runner, where the off-thread read finishes before its result is needed.
