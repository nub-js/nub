...
Entry point for AI coding agents working in this repo, per the [`AGENTS.md` convention](https://agents.md). `CLAUDE.md` is a symlink to this file. If you are OpenAI Codex, read [`CODEX.md`](../CODEX.md) after this one — it is Codex-only and does not apply to other agents.

## Skills work for EVERY agent, not just Claude (Codex included)

Task playbooks ("skills") live as plain-markdown `SKILL.md` files under `.claude/skills/<name>/SKILL.md`, each with a frontmatter `description` naming when to use it. A skill's steps are authoritative over your default approach for that task.
...
- **`remote-build`** — run a cold build, `clippy --all-targets --all-features`, a full `cargo test`, a `release` build, or an ad-hoc fixture script (`--job adhoc --script <file>`) on an ephemeral GCE spot VM instead of the dev Mac. **The VM is the DEFAULT home for every heavy or cold job and for test runs that need no macOS-specific behavior** — the dev Mac keeps only the ~5s warm inner loop and macOS-native checks. (A macOS binary comes from `scripts/mac-build.ts`, built natively on a real macOS runner.)
...
The DEFAULT home for a build, a gate, or a test run is off-host — a committed test that CI runs, or an ephemeral spot VM through `remote-build`. The dev Mac is routinely saturated by its own build fleet, and a contended box makes the loop slow.

Reach for the local box only when it gives you something off-host cannot: the ~5s warm incremental rebuild you are actively iterating against, or genuinely macOS-native behavior. Everything else goes off-host. (Decided 2026-09-01; restated 2026-09-03 because the default kept reverting in practice.)

- **Off-host, in preference order.** (1) **Write the check as a TEST and let CI run it** — one artifact both verifies this change and prevents the next regression, so prefer it whenever the behavior is expressible as a test. (2) `nub scripts/remote-build.ts --job clippy|test|adhoc --detach`, collected with `--attach <vm-name>` — byte-identical CI invocations, a few cents each, and every builder brings its own disk. (3) A branch-scoped CI probe for real macOS / Windows / Linux-arch behavior (the `ci-adhoc-test` skill) — no pull request required.
- **Do not run these locally without a stated reason:** a cold build, `cargo clippy --all-targets --all-features`, a full `cargo test`, a `release` build, or a multi-fixture sweep. "The box is otherwise idle" is a reason; "it was quicker to type" is not.
- **A local run you do make stays scoped** — `--profile fast`, the touched crates, the one test file. Never the whole workspace out of habit.
- **Carry this into every dispatch prompt that builds or tests.** A sub-agent inherits the skill LIST but no skill BODY and none of this reasoning, so it defaults straight back to `cargo test` on the dev Mac unless the prompt says otherwise.

Mechanics live in the skills: `remote-build` (VM jobs, `--detach`/`--attach`, the ad-hoc script contract), `ci-adhoc-test` (real-OS probes with no PR), `dev-loop` (the warm local loop that remains), and step 3 of the [pre-push verification loop](#implementation-quality-discipline).

## PR CI is opt-in — ask for a run with the `ci` label

Opening a pull request runs nothing. Pushing to one runs nothing. Every workflow that watches pull requests triggers on `labeled` alone and re-checks the label name, so CI happens when somebody asks for it:

```sh
gh pr edit <n> --add-label ci     # requests a full run against the current head
```

`ci-optin.yml` takes the label straight back off, so the label is a button rather than a state and the same command asks for the next round. Nothing else changed: `push: main` and the nightly schedules are untouched, so trunk keeps full automatic coverage and `trunk-red.yml` still reports what a skipped pull-request run would have caught.

**Why:** a review round used to re-run and then cancel the whole cross-platform matrix on every push. Most of the runner pool went on runs that were superseded before they finished.

- **Ask LAST, not first.** The label is worth spending once the branch is one you would merge — after the review rounds, not before the first one. Requesting a run and then pushing three more commits rebuilds the exact churn this replaced.
- **A pull request with no checks is UNVERIFIED, and the tooling says so.** `scripts/ci-watch.ts` and `scripts/merge-cascade.ts` both gate on the `CI gate` aggregate being present and green. A skipped check ran nothing, so it verifies nothing, and neither do the checks that arrive on their own — Vercel and the review bots are GitHub Apps rather than Actions workflows, so they still run on every pull request and still go green on one that has had no CI at all. Three green checks is not a verified branch. Never read "no checks reported" as "nothing to wait for".
- **Every job carries the guard, and `scripts/ci-optin.test.mjs` enforces it.** A `labeled` event fires for ANY label, so a job without the `github.event.label.name == 'ci'` check would launch the matrix whenever somebody tagged a pull request `bug`. Add a job to a pull-request workflow and the guard comes with it.
- **A contributor cannot request their own run.** Labelling needs write access, so an external pull request waits for a maintainer to ask for CI — one more reason to look at fork pull requests promptly.
...
- **The PM CLI frontend mirrors pnpm's grammar, exclusively** — not npm, not yarn, not bun. When adding or checking a flag/alias/positional on a PM verb, the only question is *does real pnpm accept it?* No npm-isms (`--omit`, `--no-save`, `-S`/`--save`, npm's `-w <name>`/`--workspaces` selectors). Gotcha: pnpm `-w` is `--workspace-root` (boolean); member selection is `--filter`, recursion is `-r`.
  - **Deliberate exceptions (2026-06-20):** (1) the top-level file run `nub <file>` is nub's own surface; (2) `nub upgrade` is nub's SELF-UPDATE, not pnpm's `upgrade`→`update` alias — `nub upgrade <pkg>` does not route to `update`; (3) no implicit script shortcuts, ever — always explicit `nub run <script>`, never `nub test`/`nub start`/`nub t`; (4) `nub init` is nub's own TypeScript-first scaffold (2026-07-21), takes no positionals, and hints `nubx create-<template>` for templates (`internal/commands/init.md`, `crates/nub-cli/src/init.rs`); (5) Nub's config surface uses project scope by default, `--global` / `nub global config` for user scope, and `config init` for a commented `nub.jsonc`; it does not expose pnpm's `--location`. Protected npm credentials are the safety exception: they default to the user `.npmrc`, with explicit `--local` required for a project write. Everything else mirrors pnpm.
...
- **`node` PATH-hijack contract (under active design; the `node-hijack-compat-design` notes are internal and outside this graph):** an invocation of `node` through nub's shim respects node/PATH semantics; the hijack's legitimate job is version resolution and pinning. Augmentation belongs to the `nub`/`nubx` entrypoints.
- **v0 verb surface:** `nub <file>` · `run` · `watch` · `nubx` / `nub dlx` / `nub x` · `upgrade` · `init` · `nub node` (version management: `install`/`ls`/`uninstall`/`pin`, `internal/commands/node-versions.md`) · the pnpm-compatible PM (`install`/`i`/`ci` plus `add`/`remove`/`update`/`import`/`dedupe`/`pm …`, engine-routed through `pm_engine::ENGINE_VERBS`; some families still filling in). `nub watch` ships in restart-mode on top of Node's `--watch`. There is no `nub node <file>` passthrough — `nub node` is the version-management namespace, so `nub node <file>` is an error.
- **`--node` and `NODE_COMPAT` are both live and compose** (either forces compat mode: zero augmentation, version provisioning stays on). `--node` works on the top-level file run, `nub run`, `nubx`, and the `node`-hijack path (`node --node <file>`). A truthy `NODE_COMPAT` (`1`/`true`/`yes`, case-insensitive) is the persistent, tree-wide equivalent, inherited by every descendant `node`/`nub`. Decision record: `internal/commands/node.md`.

## The repo is PUBLIC — never commit internal discussion

`github.com/nubjs/nub` is a public repo, and everything committed survives in git history even after deletion.

Never put internal product, marketing, or competitive discussion into anything git tracks — code, comments, commit messages, `tests/**` READMEs, any tracked markdown. Specifically banned:
...
All internal deliberation lives in the gitignored `.frizz/`, `epics/`, and `internal/`. When in doubt, leave it out of the commit. **`wiki/` is the exception and it is TRACKED and world-readable** — it is the published design and research corpus described below, so everything banned above is banned there.

## The knowledge graph in `wiki/`

`wiki/` is a [lat.md](https://github.com/1st1/lat.md) knowledge graph: cross-linked markdown recording what Nub does and why — the design decisions and the measured research behind them. This file lives in it.
...
**Resolving an `AGENTS.md` conflict: keep the symlink, and move the hunk in here.** This file IS `wiki/agents.md`; the repo root holds a symlink to it, which is what puts the entry point inside the graph. Anything branched before that move edits `AGENTS.md` as a regular file, and git cannot auto-merge a regular-file/symlink pair — you get `CONFLICT (distinct types)` and git parks the other side at `AGENTS.md~<ref>`. Take the edit out of that file, apply it here, then delete it. Keeping the symlink and dropping the hunk loses the change with no signal at all, because `lat check` is green either way — the content is simply absent. The opposite mistake — restoring a regular file — is caught for you at commit time by `.githooks/pre-commit`, again at push, and again in CI; nothing can catch this one.
...
- **Update the graph in the same commit as the behavior.** A doc naming a symbol you just renamed now fails a gate instead of rotting quietly. Three run it for you, cheapest first: `.githooks/pre-commit` when the staged changes touch `wiki/` or the lat skill, `.githooks/pre-push` on every push (which is what catches a symbol rename, since no doc was edited), and the `lat-check` job on pull requests to `main`. Run `npm run lat:check` yourself to see it sooner. Both hooks warn rather than block if the checker cannot run, and both take `NUB_SKIP_LAT_CHECK=1`.
...
- [[design/architecture]] — the plain-Node-plus-extension-surface test.
- `internal/philosophy.md` (additivity) — nub adds behavior; it never modifies existing Node semantics.
- `internal/PLAN.md` — the v0.1 manifest, Phase 1 / Phase 2 split, reversibility filter.
- `internal/whitepaper.md` §"Zero Nub-specific APIs, zero lock-in".

### The brand boundary — public surfaces only (internals are exempt)

The boundary protects **public-facing surfaces only** — anything a user imports, installs, types, or authors as configuration, or any API they call by a documented name.

**Internal mechanism is explicitly exempt:** error/warning codes, env vars nub sets for its own cross-process or shim plumbing, `globalThis.__nub*` sentinels, cache/dir names. Don't chase the brand out of internals. On the public surface the rules are absolute — no public nub-branded API even behind a flag.
...
- **No `@nub/*` npm scope** — the bare `@nub` scope is unused entirely. Everything ships under the project's own `@nubjs` org; see [`nub`-named npm packages](#nub-named-npm-packages-explained) and `internal/philosophy.md` (the brand boundary).
...
- **Prefer neutral names for USER-AUTHORED config, with one file exception.** nub reads a neutral `"tasks"` field rather than a `"nub"` field; do not add a `"nub"` package.json namespace or a `nub.toml`. The deliberate exception is project-root `nub.jsonc` — settled; do not reinterpret this preference as grounds to rename or remove it. nub may adopt another vendor-neutral, any-tool-could-implement-it field by decision record (currently `"tasks"`, `internal/commands/tasks.md`).
  - **An `exports` condition named `nub` is NOT that prohibited field, and reviews keep reading it as one.** The rule bans a `"nub"` config NAMESPACE holding nub's settings. A runtime key is the opposite shape: a slot in the cross-tool `exports` grammar that `bun`, `deno`, `workerd` and `react-server` all occupy, where the `default` branch keeps serving every other runtime. It creates no lock-in, which is what the boundary protects. The CLI sets the condition; the standalone loader deliberately does not, so a file under `@nubjs/loader` resolves the same as under tsx.
...
Web-platform and runtime APIs (Worker, HTMLRewriter, fetch, streams, …) are implemented to WHATWG/TC39/WinterTC spec, or to Cloudflare Workers semantics where there is no formal spec (the documented HTMLRewriter exception).

**Never implement a Bun-specific non-spec behavior for parity's sake.** If a user asks for one later it is considered individually, on its merits. Bun remains useful as a reference for what's possible and as a differential test oracle — not as a spec.
...
Two forms, and which one to use is decided by whether you are writing prose or naming a thing the machine reads.
...
- **Stale-doc warning:** `internal/architecture.md`'s "no own package manager" line and the augmenter/fork section's Node-only framing predate the vendored PM. Trust this section and the code.
...
Most wrong answers here trace to one habit: confirming that a thing EXISTS instead of confirming it is the thing you MEANT.

A path that matches, a search that returns hits, a pipeline that exits 0, an authoritative-sounding doc comment, a filter that yields a tidy split, a results file one stage before the gates — each is a *plausible representation* of the truth, not the truth. **Name the artifact a claim rests on, and ask whether it is downstream of the question.**
...
The highest-yield way to find correctness bugs in nub is a **differential fixture**: a minimal fixture isolating ONE behavior, run against nub AND the reference tools it claims parity with (npm / pnpm / bun / node) on identical inputs.

The divergence is the finding. This beats happy-path app rounds by a wide margin. "nub does X" is unverified until "…and npm/pnpm/bun do Y on the same fixture" is in hand.
...
nub's behavior splits by **tier**, and the two take different code paths and break differently, so a green run on one modern Node routinely masks compat-tier and floor-only defects.

The fast tier is Node 22.15+ with sync `module.registerHooks`; the compat tier is 18.19–22.14, running an async loader-worker via `module.register`.

nub discovers its Node from `PATH`, so the cheapest way to pin a version is `PATH="$HOME/.nvm/versions/node/v20.19.0/bin:$PATH" nub …`. Sweep several to cover both tiers, and use Docker for Linux + floor confirmation. **Do not claim cross-version support from one Node.**
...
- **Verify locally before pushing — don't outsource verification to CI.** A requested run is ~5 workflows × the 8-platform matrix (~30–40 min each), and several pull requests asking for round after round saturate the runner pool so head-commit jobs queue for hours. Get it green locally, then ask for ONE run ([PR CI is opt-in](#pr-ci-is-opt-in--ask-for-a-run-with-the-ci-label)). The loop, in your worktree:
  ...
  2. **Run exactly what CI runs** for the cheap gates: `NUB_ALLOW_INCOMPLETE_RUNTIME=1 cargo clippy --all-targets --all-features --profile fast -- -D warnings` (a scoped `-p` without `--all-targets` misses test-code lints), `cargo fmt --check`, and the scoped `cargo test` for what changed. Match the invocations in `.github/workflows/ci.yml`. The env var is what CI's clippy job and `make verify` set: `--all-features` turns on `embed-runtime`, whose `build.rs` demands a fully staged `runtime/`, and a lint ships nothing so it opts out instead of vendoring npm packages into your worktree. Never set it for a binary you intend to run, install, or ship. **It covers only HALF of that demand, and the other half is the one that fails you:** the env check at `crates/nub-core/build.rs:84` gates the vendored `node_modules` list alone, while the five-entrypoint integrity-hash loop further down the same file is unconditional and reads `runtime/addons/nub-native.node` — gitignored, so absent in a fresh worktree, and the panic names integrity hashing rather than the env var, which reads like a real build break. CI stages a placeholder for exactly this and so must you: `mkdir -p runtime/addons && printf 'placeholder-addon' > runtime/addons/nub-native.node`. **Delete it again before any dev-profile build or run** — `embed-runtime` is OFF by default, so `find_preload` walks to the in-repo `runtime/` and a dev binary would `dlopen` your placeholder for real. `--profile fast` on the lint gates is load-bearing, not a shortcut: it is what CI's check and clippy jobs run, and it keeps the gates in the same artifact universe as the dev loop instead of driving a second full dependency build under `dev`. `fast` inherits `dev`, so debug-assertions, overflow checks, and opt-level are identical — only debuginfo differs, which no lint reads. `cargo test` stays on the DEFAULT profile, matching CI's test jobs.
  ...
  4. **End-to-end test the specific functionality with a tmp fixture** — build the binary and exercise the actual feature against a throwaway fixture, diffing against the reference tool where parity is claimed. "Tests pass" is not "the feature works." **This too defaults to the builder VM when the behavior needs no macOS specifics**: write the probe as a script and dispatch it with `nub scripts/remote-build.ts --job adhoc --script <file> --detach` — it runs at the synced repo root with `NUB_BIN` naming a freshly built binary (the `ad-hoc-test` skill carries the method, the `remote-build` skill the mechanics). Keep the fixture local when you are iterating tightly against the warm binary or probing macOS-native behavior.
  ...
  7. **A change touching `site/**` is gated by `.githooks/pre-push`,** which runs the same `next build` Vercel runs whenever the pushed range touches the site (~20s; a Rust-only push pays ~35ms and skips). It auto-installs site deps on first use in a worktree, and never blocks on infrastructure — only on a real build failure. For the inner loop use `pnpm run typecheck` in `site/` (~2s). This matters because `ci.yml` only fires on `push: main` / `pull_request: main`, so a long-lived feature branch gets NO CI and Vercel would otherwise be the first thing to type check the site — reporting by email, after the fact.
     ...
     The same hook also runs `lat check` on every push, and `.githooks/pre-commit` runs it when the staged changes touch `wiki/` or the lat skill. Both take `NUB_SKIP_LAT_CHECK=1`, which is separate from the site gate's `NUB_SKIP_SITE_CHECK=1`.
...
Conventions specific to the docs site, on top of the shared guide above.
