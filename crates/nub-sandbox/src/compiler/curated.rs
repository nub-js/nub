    ...
    if let Reach::Scopes(scopes) = &grant.write {
        for scope in scopes {
            match scope {
                ...
                // A catalog `userHome` grant is literal. The broad capability includes
                // credential-bearing files; policy authors must choose narrower explicit paths
    ...
    if let Reach::Scopes(scopes) = &grant.read {
        for scope in scopes {
            match scope {
                ...
                // A catalog `userHome` grant is literal on both access axes.
                Scope::UserHome => {
                    for glob in defaults::subtree_globs(&homes.home.to_string_lossy()) {
                        rules.push(rule(&glob, FsAccess::Read));
                    }
                }
