    ...
    if crate::catalog_override::v2_in_force() {
            ...
            .unwrap_or_else(|| std::borrow::Cow::Owned(crate::catalog_v2::baseline_caps()));
        {
            ...
            if out.write_disk {
                ...
                relax_fs_read_to_disk(&mut policy);
            }
        }
    }
...
///
/// A full-disk write clears the rules and changes the default to Allow. It is unconfined.
/// The distinct `read:"disk"` tier remains default-deny but grants root read literally; it does
/// not claim to preserve a credential exclusion.
///
...
///
/// A catalog `read: "disk"` grant is literal broad access. It is represented by a
/// whole-filesystem read rule while the default remains deny, so write access still
/// depends on an explicit writable rule. Backends must either lower this root-read
/// rule or report that the requested read capability was not enforced; they must not
/// substitute a secret-excluding complement.
pub fn relax_fs_read_to_disk(policy: &mut SandboxPolicy) {
    policy.fs.rules.entries.splice(
        0..0,
        [FsRule {
