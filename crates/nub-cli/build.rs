...
fn main() {
    ...
    // The recorded rerun-if-changed path above is absolute, so under the shared
    // target dir cargo keeps a SIBLING worktree's baked docs and never reruns
    // this script (observed 2026-09-07: a branch that moved pages tested a
    // sibling's tree). `CARGO_MANIFEST_DIR` cannot fix that: the cargo book says
    // rerun-if-env-changed tracks only variables cargo itself RECEIVES, never
    // the ones it sets for build scripts. `scripts/rust-build.sh` therefore
    // exports NUB_DOCS_KEY, a content hash of the docs tree, so the bake reruns
    // exactly when the docs differ from the sharer that produced it.
    println!("cargo:rerun-if-env-changed=CARGO_MANIFEST_DIR");
    println!("cargo:rerun-if-env-changed=NUB_DOCS_KEY");
